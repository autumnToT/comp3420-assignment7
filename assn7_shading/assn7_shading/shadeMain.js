...
function bindVAO (shape, program) {
    ...
    theVAO.numIndices = shape.indices.length;
...
function setUpPhong(program) {
    ...
    // Light properties
    gl.uniform3fv(program.lightPosition, [7.0, 5.0, 5.0]);
    gl.uniform3fv(program.ambientLight, [0.4, 0.4, 0.4]);
    gl.uniform3fv(program.lightColor, [1.5, 1.5, 1.5]);

    // Material properties
    gl.uniform3fv(program.baseColor, [1.0, 0.2, 0.2]);
    gl.uniform3fv(program.specHighlightColor, [1.0, 1.0, 1.0]);

    // Phong coefficients
    gl.uniform1f(program.ka, 0.3); // Ambient coefficient
    gl.uniform1f(program.kd, 0.8); // Diffuse coefficient
    gl.uniform1f(program.ks, 0.7); // Specular coefficient
    gl.uniform1f(program.ke, 40.0); // Shininess (specular exponent)
...
function setUpCamera(program) {
    ...
    glMatrix.mat4.perspective(projMatrix, radians(45), gl.canvas.width / gl.canvas.height, 0.1, 100.0);
